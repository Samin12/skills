...
Editable HyperFrames source for the ten 1.6-second chapter cards inserted into the Borumi project `Fable 5.1` (`5689`). The cards use the reusable grid-and-typed-title treatment from the parent `create-chapter-transitions` skill.
...
Scene 2 uses a precomposed surgeon clip with Samin's presenter camera in the top-right corner and `EDITED BY FABLE 5.1` in the upper-left. The ten rendered cards are 1920×1080, H.264, 30 fps, and 1.6 seconds each. The corrected Borumi edits were committed as `cc96` (standalone chapter gaps, bass click, and Scene 2 composite) and `25c6` (conflicting overlay cleanup).
